...
public class Table {
    ...
    public boolean enterPiece(Piece piece) {
        ...
        for (Coordinate coordinate : piece.getPieceCoordinates()) {
            ...
            System.out.println("Height - 1: " + (this.height - 1) + "# MaxHeight: " + piece.getMaxHeight());
            System.out.println("Width / 2: " + (this.width / 2) + "# MAxWidth / 2: " + (piece.getMaxWidth() / 2));
            field = this.fields[0
                    + coordinate.getX()][(this.width / 2)
                    + coordinate.getY()
                    - (piece.getMaxWidth() / 2)];
    ...
    public boolean move(Piece piece) {
        ...
        for (Coordinate coordinate : piece.getPieceCoordinates()) {
            ...
            field.setColor(PieceColor.DEFAULT);
        }
    ...
    /**
     * ellenőrzi, hogy tele van-e a sor, ha igen akkor teljesen kiűríti azt,
     * majd lejebb tol minden felette levő sort egyel a shiftRows fv-ben
     *
     * @param height - melyik sorban
     * @return 
     *
     */
